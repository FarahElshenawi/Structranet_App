...
export default function ChatPage() {
  ...
  useEffect(() => {
    // H7: Restore last active chat from MongoDB on mount
    getChats().then((chats) => {
      setHistory(chats);
      // Restore the most recent chat if it has messages
      if (chats.length > 0) {
        const mostRecent = chats[0]; // already sorted by most recent
        if (mostRecent._id || mostRecent.id) {
          getChat(mostRecent._id || mostRecent.id).then((full) => {
            const msgs = full.messages || [];
            if (msgs.length > 0) {
              const restored = [];
              msgs.forEach((m) => {
                if (m.role === "user") {
                  restored.push({ role: "user", text: m.content, id: Date.now() + Math.random() });
                }
              });
              if (restored.length > 0) {
                setMessages(restored);
                setChatId(mostRecent._id || mostRecent.id);
                if (full.sessionId) setSessionId(full.sessionId);
              }
            }
          }).catch(() => {}); // non-critical
        }
  ...
  const handleSend = useCallback(async (text) => {
    ...
    // 1. Push user bubble with optimistic flag (H6)
    const msgId = Date.now();
    setMessages((prev) => [...prev, { role: "user", text, id: msgId, optimistic: true }]);
    // 2. Push empty AI bubble (will fill via SSE + agentChat response)
    setMessages((prev) => [...prev, { role: "ai", id: msgId + 1 }]);
    ...
    try {
      ...
      // H6: Optimistic update — remove optimistic flag on success
      setMessages((prev) => prev.map(m => m.id === msgId ? { ...m, optimistic: false } : m));
      ...
      stopStream();
      reset();
      // H6: Mark user message as failed (remove optimistic, add failed flag)
      setMessages((prev) => prev.map(m => m.id === msgId ? { ...m, optimistic: false, failed: true } : m));
  ...
  // ── Retry a failed message (H6) ──────────────────────────────
  const handleRetry = useCallback((failedMsgId, text) => {
    // Remove the failed message and its associated AI bubble, then re-send
    setMessages((prev) => {
      const idx = prev.findIndex((m) => m.id === failedMsgId);
      if (idx < 0) return prev;
      // Remove the failed user message and the AI bubble that follows it
      const next = [...prev];
      next.splice(idx, 1);
      // Also remove the adjacent AI placeholder if present
      if (idx < next.length && next[idx].role === "ai") {
        next.splice(idx, 1);
      }
      return next;
    });
  ...
  return (
    <div style={{
              ...
              if (msg.role === "user") {
                ...
                return (
                  <div key={idx} style={{
                    ...
                    <div style={{
                      background: isFailed ? "#FEE2E2" : G,
                      color: isFailed ? "#DC2626" : "white",
                      ...
                      {isFailed && (
                        <div style={{ marginTop: 6, fontSize: 12, fontWeight: 600 }}>
                          <span style={{ opacity: 0.8 }}>Failed to send. </span>
                          <span
      ...
      {/* ── TopologyViewer overlay (wrapped in error boundary) ── */}
      {/* Only pass topology, requirements, onClose — NOT onEdit/onApprove/isReview */}
      {topologyOpen && topology && (
        <TopologyErrorBoundary onClose={() => setTopologyOpen(false)}>
          <TopologyViewer
            topology={topology}
            requirements={requirements}
