...
/* ═══════════════════════════════════════════════════════════════════════
   APPLIANCE CATALOG — Native Express endpoint (FastAPI proxy removed)
   Returns an empty catalog since the Python AI engine handles catalog
   logic internally. The frontend's getCatalog() still works but gets
   an empty array instead of a 502 error.
   ═══════════════════════════════════════════════════════════════════════ */

app.get("/api/catalog", requireAuth, async (req, res) => {
  res.json({ appliances: [], templates: [] });
});
