...
V2: adds security_profile field ("none" | "basic" | "enterprise").
"""
...
class PreflightProfile:
    ...
    # ── NEW ──────────────────────────────────────────────────────────────────
    security_profile: str = "none"   # "none" | "basic" | "enterprise"
...
def profile_from_dict(data: Dict[str, Any]) -> PreflightProfile:
    return PreflightProfile(
        ...
        security_profile=str(data.get("security_profile", "none")),
    )
...
def collect_profile_interactive() -> PreflightProfile:
    ...
    if require_map:
        print("  Enter template=image pairs (empty line to finish):")
        while True:
    ...
    # ── Security profile selection (NEW) ────────────────────────────────────
    print("\n  Security profile:")
    print("    none       — no hardening (pure lab / universal)")
    print("    basic      — SSH, AAA, banners, NTP, Syslog on every router")
    print("    enterprise — full ZBF, ACLs, DAI, DHCP Snooping, SNMPv3,")
    print("                 redundancy, HSRP, uRPF, OSPF auth")
    sec_raw = input("  Choose [none/basic/enterprise] (default: none): ").strip().lower()
    security_profile = sec_raw if sec_raw in ("none", "basic", "enterprise") else "none"
...
def check_topology_compatibility(
    ...
    if profile.require_template_image_map:
        ...
        for node in nodes:
            ...
            if not template:
                issues.append(
                    f"Node '{name}' ({ntype}) is missing template_name, "
                    "cannot verify image mapping."
                )
                continue
            if template not in mapping:
                issues.append(
                    f"Node '{name}' template '{template}' is not in "
                    "preflight template_image_map."
                )
    return issues
